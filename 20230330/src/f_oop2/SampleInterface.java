...
public interface SampleInterface {
	...
	//인터페이스의 모든 멤버 변수는 public static final 제어자가 붙는다.
...
class SampleImplement implements SampleInterface, Sampleinterface2{
	...
	//만약 인터페이스에서 강제하고 있는 메서드를 구현하지 않으면 이 어플리케이션은 컴파일조차 되지 않는다.
	//인터페이스는 몸체가 없는 메서드들의 모임이다. 클래스를 구현하기 전에 만들 메서드를 먼저 정해놓는 것이다.
